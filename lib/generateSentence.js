...
export async function generateSentence({ word, level, topic }) {
  if (!process.env.GEMINI_API_KEY) {
    throw new Error("Missing GEMINI_API_KEY env variable");
  }

  if (!word || !level || !topic) {
    throw new Error("Parameters 'word', 'level' and 'topic' are required");
  }
  ...
  const response = await ai.models.generateContent({
    ...
    contents: `Write one sentence with English Level ${level} with word '${word}' and in the context of the topic '${topicPrompt}'. The sentence must be natural and concise. Return only the sentence.`,
  });
